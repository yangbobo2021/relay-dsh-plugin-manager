...
async function searchProvider(
  ...
  try {
    const result = await provider.search({ query, maxResults, signal: controller.signal })
...
export async function searchPlugins(
  ...
  const settled = await Promise.allSettled(providers.map(async provider => ({
    ...
    rows: await searchProvider(provider, query, maxResults, options.signal, timeoutMs),
  })))
  ...
  const projects = new Map<string, SearchResult['candidates'][number] & { rank: number }>()
  ...
  const candidates = [...projects.values()]
    .sort((left, right) => left.rank - right.rank || left.packageName.localeCompare(right.packageName))
    .slice(0, maxResults)
    .map(({ rank: _rank, ...candidate }) => candidate)
  return { query, candidates, providerErrors, rejectedCandidates }
}
