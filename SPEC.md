...
| PM-022 | Preserve manifest peer-dependency metadata during source inspection. A multi-install plan reports peer dependencies absent from both the current profile and the requested set, deduplicated with their ranges, dependents, and suggested npm source. Planning never silently adds an unrequested companion plugin. |
